<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">

  <title>TWUISEO - Marketing Digital, desenvolvimento de sites e aplicativos</title>
  <link rel="stylesheet" href="{{ asset('resources/css/animate.css') }}">
  <link rel="stylesheet" href="{{ asset('resources/css/bootstrap.min.css') }}">
  <link rel="stylesheet" href="{{ asset('resources/css/bootsnav.min.css') }}">
  <link rel="stylesheet" href="{{ asset('resources/css/style.min.css') }}">
  <link rel="stylesheet" href="{{ asset('resources/css/twuiseo.min.css') }}">
  <link rel="stylesheet" href="{{ asset('resources/font-awesome/css/font-awesome.min.css') }}">
  <link rel="stylesheet" href="{{ asset('ionicons/dist/css/ionicons.min.css') }}">
  <link rel="stylesheet" href="{{ asset('resources/css/fonts.min.css') }}">
  <!-- Plugins -->
  <link rel="stylesheet" href="{{ asset('resources/plugins/leader/leader-line.min.css') }}">
  <link rel="stylesheet" href="{{ asset('resources/plugins/jquery-elevator/jquery.elevator.min.css') }}">
  <link rel="stylesheet" href="{{ asset('resources/plugins/jquery-sweet-modal/jquery.sweet-modal.min.css') }}">
  <link rel="stylesheet" href="{{ asset('resources/css/plans.min.css') }}">
</head>
<body>

<main>
  <section class="home" id="home">
    <div class="container">
      <div class="brand-twuiseo">

        <svg
            xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink"
            width="223px" height="191px">
          <image x="0px" y="0px" width="223" height="191"
                 xlink:href="data:img/png;base64,"/>
        </svg>
      </div>
      <h1 class="title">TWUISEO</h1>
      <h3 class="title2">A primeira impressão é a que fica</h3>
    </div>
  </section>
  <!-- Start Navigation -->
  <nav class="navbar navbar-twuiseo navbar-fixed-bottom bootsnav">

    {{--<!-- Start Top Search -->--}}
    {{--<div class="top-search">--}}
    {{--<div class="container">--}}
    {{--<div class="input-group">--}}
    {{--<span class="input-group-addon"><i class="fa fa-search"></i></span>--}}
    {{--<input type="text" class="form-control" placeholder="Pesquisar">--}}
    {{--<span class="input-group-addon close-search"><i class="fa fa-times"></i></span>--}}
    {{--</div>--}}
    {{--</div>--}}
    {{--</div>--}}
    {{--<!-- End Top Search -->--}}

    <div class="container">
    {{--<!-- Start Atribute Navigation -->--}}
    {{--<div class="attr-nav">--}}
    {{--<ul>--}}
    {{--<li class="search"><a href="#"><i class="fa fa-search"></i></a></li>--}}
    {{--</ul>--}}
    {{--</div>--}}
    {{--<!-- End Atribute Navigation -->--}}

    <!-- Start Header Navigation -->
      <div class="navbar-header">
        <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#navbar-menu">
          <i class="fa fa-bars"></i>
        </button>
        <a class="navbar-brand twuiseo-scroll" href="#home">
          <svg style="margin-top: -10px"
               xmlns="http://www.w3.org/2000/svg"
               xmlns:xlink="http://www.w3.org/1999/xlink"
               width="70" height="70">
            <image x="0px" y="0px" width="70" height="70"
                   xlink:href="data:img/png;base64,"/>
          </svg>
        </a>
      </div>
      <!-- End Header Navigation -->

      <!-- Collect the nav links, forms, and other content for toggling -->
      <div class="collapse navbar-collapse" id="navbar-menu" style="font-family: Montserrat">
        <ul class="nav navbar-nav navbar-right" data-in="fadeInDown" data-out="fadeOutUp">
          <li class="active"><a class="twuiseo-scroll" href="#home">Home</a></li>
          <li><a class="twuiseo-scroll" href="#services">Serviços</a></li>
          <li><a class="twuiseo-scroll" href="#design">Design</a></li>
          {{--<li><a class="twuiseo-scroll" href="#news">Novidades</a></li>--}}
          @if (Route::has('login'))
            @auth
            <li><a href="{{ url('/home') }}"><b> <i class="fa fa-dashboard"></i> Home</b></a></li>
          @else
            <li><a href="{{ route('register') }}"><b>Registrar</b></a></li>
            <li><a class="twuiseo-scroll" href="{{ route('login') }}"><i class="ion-md-person"></i> Login</a></li>
            @endauth
          @endif
        </ul>
      </div><!-- /.navbar-collapse -->
    </div>
  </nav>
  <!-- End Navigation -->

  <div class="clearfix"></div>

  <section id="services">
    <article>
      <div class="row" style="text-align: center">

        <div class="container">
          <div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
            <div class="box">
              <header>Lojas Virtuais</header>
              <main>
                <img src="{{ asset('resources/images/services/ecommerce.png') }}" class="img-service">
              </main>
              <article>
                <p style="text-align: center;">Lojas Virtuais: Venda seus produtos online</p>
                Hoje em dia é cada vez mais comum você comprar pela internet. Atinja um público maior do que o da sua
                região
                podendo aumentar as suas vendas. Venda seus produtos 24 horas por dia!
              </article>
              <footer>
                <a class="links-tools" onclick="modal('modalTwuiseo')" id="modalTwuiseo"
                   data-twuiseo-title="Loja virtual, preciso mesmo?"
                   data-twuiseo-content="
                 <p><i>Lhe daremos alguns motivos para você ter sua loja virtual</i></p>
                       <p class='modal-title'><i>1. Atendimento 24 horas:</i>
                       <p>
                        &nbsp;&nbsp;&nbsp; Já pensou em vender dia e noite, sem parar? Pois ao abrir uma loja virtual é exatamente
                       o que você vai fazer! Com um e-commerce, não existe horário para fechar. Lá, seus produtos estão disponíveis a qualquer
                       hora e em qualquer lugar.
                       </p>

                       <p class='modal-title'><i>2. Comodidade:</i>
                       </p>
                       <p>
                        &nbsp;&nbsp;&nbsp; Com uma loja virtual você envia e o seu cliente recebe sem sair de casa.
                        Hoje em dia a comodidade é tudo, seus clientes podem estar na mesma cidade, mas eles preferem comprar sem sair de casa.
                        Pessoas da sua região e de todo o território Brasileiro poderá comprar produtos de qualidade sem sair de suas cadas.
                        É essa comodidade que fez com que <b>48 milhões</b> de pessoas fizessem ao menos uma compra via e-commerce em 2016,
                        um incremento de 22% no número de consumidores on-line me comparação a 2015.
                       </p>

                       <p class='modal-title'><i>3. Promoções:</i></p>
                       <p>
                        &nbsp;&nbsp;&nbsp; Um dos grandes atrativos do e-commerce é a possibilidade de criar promoções rapidamente e divulgá-las
                       pela webb em questão de poucas horas. Você não precisa de criar nenhum material impresso para dar conhecimento aos consumidores.
                       </p>

                       <p class='modal-title'><i>4. Custos reduzidos:</i></p>
                       <p>
                        &nbsp;&nbsp;&nbsp; Abrir uma loja virtual exige menos investimentos que um negócio físico, uma vez que não demanda
                        infraestrutura voltada para visitação de clientes, tampouco vendedores à disposição.
                       </p>

                       <p class='modal-title'><i>5. Aumento das vendas:</i></p>
                       <p>
                        &nbsp;&nbsp;&nbsp; Para quem já possui um negócio físico, abrir uma loja virtual é uma oportunidade de ouro para ampliar os
                        horizontes e, claro, as vendas.
                       </p>

                       <hr style='box-shadow: 0px 1px 0px #2b2e37;border-color: #1c1e23'>
                       <div style='text-align: center'>
                        Então não perca tempo e abra uma loja virtual agora a mesmo! ;)
                       </div>
                   ">
                  <img src="{{ asset('resources/images/services/more.svg') }}" class="more">
                </a>


              </footer>
            </div>
          </div>

          <div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
            <div class="box">
              <header>Aplicativos móveis</header>
              <main>
                <img src="{{ asset('resources/images/services/phone.png') }}" class="img-service">
              </main>
              <article>
                <p style="text-align: center;">Faça de seu site um app</p>
                O Brasil já é o 6º maior mercado mundial de smartphones. Na mesma pesquisa temos que 41% dos brasileiros
                que possuem
                smartphone já realizaram algum compra online por aplicativos.
              </article>
              <footer>
                <a class="links-tools" onclick="modal('modalApp')" id="modalApp"
                   data-twuiseo-title="Porque ter um aplicativo?"
                   data-twuiseo-content="
          O Brasil já é o 6º maior mercado mundial de smartphones. Na mesma pesquisa temos que 41% dos brasileiros que possuem
            smartphone já realizaram algum compra online por aplicativos. Em novembro de 2015, 20% dos donoes de smartphones possuíam algum
            aplicativo mobile pago, e 45% realizaram compra nos aplicativos.
            <br><br>

            <i class='modal-title'>Mas eu realmente preciso de um app?</i><br><br>
            <p>
              Se você tem uma empresa que utilize algo relacionado a marcação de horários, pedidos e entregas. Nada melhor do que um aplicativo para isso
              pois um telefone todos conseguem carregar no bolso, já um computador não é tão simples...
            </p>
        ">
                  <img src="{{ asset('resources/images/services/more.svg') }}" class="more">
                </a>


              </footer>
            </div>
          </div>
          <div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
            <div class="box">
              <header>Blogs e Galerias</header>
              <main>
                <img src="{{ asset('resources/images/services/blogs.png') }}" class="img-service">
              </main>
              <article>
                <p style="text-align: center;">Mostre seus trabalhos para todos</p>
                Se você trabalha com imagens ou com blogs temos um plano perfeito para você!
                Na Twuiseo você pode mostrar seu trabalho usando um dos nossos temas feito especialmente para você.
              </article>
              <footer>
                <a class="links-tools" onclick="modal('modalTwuiseoBlogs')" id="modalTwuiseoBlogs"
                   data-twuiseo-title="Porque ter um blog/galeria?"
                   data-twuiseo-content="
          Se você trabalha com imagens ou é um fotografo (a), criamos para você uma galeria especial com tema totalmente inspirados
          em suas artes. Para assim você poder alcançar mais clientes, não somente de sua região mas sim de todo o Brasil.
          <br><br>
          Esta é uma forma de comunicação direta com seu cliente, já mostrando seu trabalho logo de cara.
        "
                >
                  <img src="{{ asset('resources/images/services/more.svg') }}" class="more">
                </a>


              </footer>
            </div>
          </div>
          <div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
            <div class="box">
              <header>SEO - Otimização de Sites</header>
              <main>
                <img src="{{ asset('resources/images/services/seo.svg') }}" class="img-service">
              </main>
              <article>
                <p style="text-align: center;">SEO: Otimização de Motor de Busca.</p>
                Tenha seu site encontrado pelos mais famosos buscadores, tais como: Google, Bing e Yahoo.
                <b>Aumente </b> a quantidade de
                <b>acessos </b>no seu site! Temos planos específicos para cada necessidade!
              </article>
              <footer>
                <a class="links-tools" onclick="modal('modalTwuiseoSEO')" id="modalTwuiseoSEO"
                   data-twuiseo-title="Porque o SEO é tão importante?"
                   data-twuiseo-content="
          Se a sua empresa quer ser encontrada na internet é necessário uma estratégia de SEO para que seu site seja indexado e encontrado
          pelo Google, Yahoo e Bing (os maiores buscadores do mundo). Não adianta nada criar um site perfeito e ninguém ter acesso a ele.<br><br>
          Encontre seu site facilmente buscando pelo nome dele no Google. Nós traçamos a melhor estratégia de Marketing Digital para seu site!
        "
                >
                  <img src="{{ asset('resources/images/services/more.svg') }}" class="more">
                </a>


              </footer>
            </div>
          </div>

          <div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
            <div class="box">
              <header>Soluções Comerciais</header>
              <main>
                <img src="{{ asset('resources/images/services/solutions.png') }}" class="img-service">
              </main>
              <article>
                Sua empresa precisa de uma loja, blog, catálogos online ou qualquer outra coisa? Parabéns, você está no
                local certo! Aqui na TWUISEO você encontra a solução perfeita para todos as suas necessidades, nós não
                vendemos apenas um site.
                <div style="text-align: center;">
                  <b>Nós vendemos soluções!</b>
                </div>
              </article>
              <footer>
                <a class="links-tools" onclick="modal('modalSolutions')" id="modalSolutions"
                   data-twuiseo-title="As melhores soluções"
                   data-twuiseo-content="
                Temos a melhor solução para você! Desde blogs, lojas virtuais, cardápios onlines, sites comerciais até aplicativos de telefones.
                Com isso nossa empresa se torna uma empresa que age de forma correta e preços justos.
">
                  <img src="{{ asset('resources/images/services/more.svg') }}" class="more">
                </a>
              </footer>
            </div>
          </div>
          <div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
            <div class="box">
              <header>Projetos Pessoais</header>
              <main>
                <img src="{{ asset('resources/images/services/themes.svg') }}" class="img-service">
              </main>
              <article>
                <p style="text-align: center;">Desenvolvimento de todos projetos que você tiver em mente</p>
                Você sempre teve uma idéia em sua mente para um site ou uma loja virtual e eles eram muito caros ou
                consumiam muito do seu tempo? Nós temos a solução especial pra você!
              </article>
              <footer>
                <a class="links-tools" onclick="modal('modalProjectsPerson')" id="modalProjectsPerson"
                   data-twuiseo-title="Projetos Pessoais" data-twuiseo-content="

              Você tem projetos pessoais parados? <br>
              Projetos que sempre quis tirar do papel?<br>
              Projetos que saíram caros em outras empresas?<br>
              <br>
              Traga-nos o seu projetos e avaliaremos todo o contexto do seu projeto e voltaremos com uma proposta bem tentadora a você! ;)
              <br>
              Entre em contato conosco para você poder nos contar o que você precisa.

    ">
                  <img src="{{ asset('resources/images/services/more.svg') }}" class="more">
                </a>
              </footer>
            </div>
          </div>
        </div>
      </div>
    </article>
  </section>

  <!--<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1513 147.33"><title>Ativo 2</title><path d="M1119,29.33c-153-66-363,0-363,0v.27c-16.9,4.48-245.72,63.26-393-.27-153-66-363,0-363,0v118H1513v-118S1272,95.33,1119,29.33Z" style="fill:#1a1a1a"/></svg>-->
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1513 156.33"><title>Ativo 5</title>
    <path
        d="M1117.52,29.33c-152.8-66-362.52,0-362.52,0v.27c-16.88,4.48-245.39,63.26-392.48-.27-152.8-66-362.52,0-362.52,0v118H1511v-118S1270.32,95.33,1117.52,29.33Z"
        style="fill:#333"/>
    <path
        d="M1119,38.33c-153-66-363,0-363,0v.27c-16.9,4.48-245.72,63.26-393-.27-153-66-363,0-363,0v118H1513v-118S1272,104.33,1119,38.33Z"
        style="fill:#1a1a1a"/>
  </svg>

  <section class="more-information-1" id="design">
    <div class="container">
      <div class="row">
        <div class="center">
          <div id="responsive-type"></div>
          <img src="{{ asset('resources/images/services/responsive.png') }}" class="responsive img-responsive-twuiseo">
          <p style="text-align: justify;font-size: 18px;line-height: 30pt">Faça com que seu site seja mais acessível.
            Com a
            contratação de qualquer plano da <b>TWUISEO <sup>&copy;</sup></b> &nbsp; você tem a responsividade todal do
            seu
            site para qualquer tela, facilitando assim a visualização do seu site tanto em Computadores como também em:
            <b>Tablets,
              Notebooks e Telefones</b>.</p>
        </div>

        <!--Start plans-->
        <h1 style="text-align: center;color: white">
          <img src="{{ asset('resources/images/others/tasks.svg') }}" style="width: 45px;margin-top: -10px;">
          Planos
        </h1>
        <div class="container">
          <div class="row">
            <div class="col-md-3 col-sm-6">
              <div class="pricingTable">
                <h3 class="title">Site</h3>
                <div class="price-value">
                  <span class="amount">R$ 70</span>
                  <span class="month">Mensal</span>
                </div>
                <ul class="pricing-content">
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Site responsivo <span
                        style="float: right">
                  <i class="fa fa-info-circle" data-twuiseo-title="O que é responsividade?" style="color: #18b2ff;"
                     id="modalResponsive" onclick="modal('modalResponsive')"
                     data-twuiseo-content="<b>Design Reponsivo</b> é uma técnica de estruturação HTML e CSS, em que o site se adapta
ao browser do usuário.

  <br><br>
  Resumindo:<br>
  É você ter seu site disponível para computadores, notebooks, tablets e celulares. Podendo assim seu site ser visível sem a necessidade de um aplicativo externo.
"
                  ></i>
                </span></li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> 1 ano de domínio
                    grátis
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> 4 páginas responsívas
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Tema personalizado
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> seusite<b>.com.br</b>
                  </li>
                </ul>
                <a class="pricingTable-signup" href="#">CONTRATAR</a>
              </div>
            </div>

            <div class="col-md-3 col-sm-6">
              <div class="pricingTable pink">
                <h3 class="title app">Aplicativo</h3>
                <div class="price-value">
                  <span class="amount" style="font-size: 45px">R$ 100</span>
                  <span class="month">Mensal</span>
                </div>
                <ul class="pricing-content">
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Lançamento grátis
                    <span style="float: right;"><i class="fa fa-info-circle" style="color: #18b2ff;" id="modalPriceApp"
                                                   onclick="modal('modalPriceApp')"
                                                   data-twuiseo-title="Lançamento na PlayStore grátis"
                                                   data-twuiseo-content="
    Na contratação do desenvolvimento de um aplicativo a Twuiseo dá de graça o lançamento na PlayStore. <br><br>

    <i>Para o lançamento de um aplicativo que seja administrável é necessário a contratação de um site para que você possa administra-lo pelo site.</i>
"></i></span></li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> 3 Telas administráves
                    <span style="float: right;"><i class="fa fa-info-circle" style="color: #18b2ff;"
                                                   id="modalPriceAppWindow" onclick="modal('modalPriceAppWindow')"
                                                   data-twuiseo-title="3 ou 4 telas para seu aplicativo"
                                                   data-twuiseo-content="
    Você escolhe se quer 3 Páginas administráveis ou se irá querer 4 telas estáticas (que não é possível a edição pelo site)
<br><br>
    <i>As páginas administráveis só podem ser feita após a contratação do site.</i>
"></i></span>
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Tema personalizado

                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> 1º Marketing Grátis

                    <span style="float: right;"><i class="fa fa-info-circle" style="color: #18b2ff;"
                                                   id="modalPriceAppMkt" onclick="modal('modalPriceAppMkt')"
                                                   data-twuiseo-title="Primeiro marketing grátis" data-twuiseo-content="
  As primeiras imagens de Marketing na PlayStore serão gratuitas pelo primeiro lançamento. <br><br>

    <i>Planos de Marketing para PlayStore pode ser adquirido separadamente.</i>
"></i></span>
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Aplicativo Modulável

                    <span style="float: right;"><i class="fa fa-info-circle" style="color: #18b2ff;"
                                                   id="modalPriceAppModule" onclick="modal('modalPriceAppModule')"
                                                   data-twuiseo-title="Aplicativo modulável" data-twuiseo-content="
  Todos os aplicativos da Twuiseo são modulável, podendo assim, ser adicionado novas funções para o aplicativo utilizando o sistema de atualizações da
  PlayStore.<br><br>

    <i>Planos de Modulação pode ser adquirido separadamente.</i>
"></i></span>
                  </li>
                </ul>
                <a class="pricingTable-signup" href="#">CONTRATAR</a>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <div class="pricingTable">
                <h3 class="title" style="font-size: 42px">SEO</h3>
                <div class="price-value">
                  <span class="amount" style="font-size: 45px">R$ 20</span>
                  <span class="month">Mensal</span>
                </div>
                <ul class="pricing-content">
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Seja encontrado na
                    web <span style="float: right"><i class="fa fa-info-circle" style="color: #18b2ff;"
                                                      onclick="modal('modalTwuiseoSEO')"></i></span></li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Estratégias
                    individuais
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Otimização do site
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Seja rankeado no
                    Google
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Site veloz
                  </li>
                </ul>
                <a class="pricingTable-signup" href="#">CONTRATAR</a>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <div class="pricingTable pink">
                <h3 class="title tools">Ferramentas</h3>
                <div class="price-value">
                  <span class="amount" style="font-size: 45px">R$ 15</span>
                  <span class="month">Mensal</span>
                </div>
                <ul class="pricing-content">
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Instalação grátis
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Personalizadas /
                    Padrão
                    <span style="float: right;"><i class="fa fa-info-circle" style="color: #18b2ff;" id="modalTools"
                                                   onclick="modal('modalTools')"
                                                   data-twuiseo-title="Ferramentas personalizadas"
                                                   data-twuiseo-content="
    As ferramentas padrão e ferramentas personalizadas são criadas na ponta do lápis para você, cliente exigente que quer qualidade.
    <br> Se não temos a ferramenta que você deseja, nós a criamos pelo mesmo preço!
    <br><br>
    <i>As ferramentas poderão ser criadas e implementadas para clientes da Twuiseo</i>
"></i></span>
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Ferr. para Lojas
                    Virtuais
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Ferr. para Blogs
                  </li>
                  <li>
                    <img src="{{ asset('resources/images/others/icon-plans.svg') }}" width="10px"> Ferr. para Sites
                    Comerciais
                  </li>
                </ul>
                <a class="pricingTable-signup" href="#">CONTRATAR</a>
              </div>
            </div>


          </div>
        </div>
        <!-- End plans-->
      </div>
    </div>
  </section>

  <section class="ad1">
    <div class="pull2" style="padding-bottom: 40px"></div>
    <div class="row">
      <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
        <h3 style="color: white;text-align:center">Dúvidas comuns</h3>
        <!-- TAB NAVIGATION -->
        <ul class="nav nav-tabs" role="tablist">
          <li class="active"><a href="#newSite" role="tab" data-toggle="tab">Porque ter um site?</a></li>
          <li><a href="#min" role="tab" data-toggle="tab">Pequenas empresas</a></li>
          <li><a href="#themes" role="tab" data-toggle="tab">Quais tipos de sites?</a></li>
        </ul>
        <!-- TAB CONTENT -->
        <div class="tab-content">
          <div class="active tab-pane fade in" id="newSite">
            <p>Atraia novos clientes para seu negócio, expanda sua empresa para a internet. Aumente suas vendas, seja
              encontrado na internet.</p>
            <p>Hoje em dia tudo gira em torno da internet, desde a busca por um endereço, uma página ou até mesmo um
              número de telefone. A melhor forma de ser encontrado na web é ter um site.</p>
            <hr style="border-top: 1px dotted #370304;margin-bottom: 0">
            <div style="opacity: 0.7;font-size: 13px">
              A TWUISEO utiliza as ferramentas mais modernas para o desenvolvimento de sites, como por exemplo sites
              responsivos. Sites que se adaptam as telas e otimizão a experiência do usuário com o seu site. Para isso
              não é necessário o recarregamento da página do usuário <b>(economiza internet 3G)</b>.
            </div>
          </div>
          <div class="tab-pane fade" id="min">
            <p style="text-align: center">Sua empresa é pequena?</p>
            <p>A melhor forma de faze-la crescer e ser conhecida é pela internet. Com um site você consegue alcançar
              além de clientes da sua cidade como também de outros cidades, estadose e até países.
              <br>
              Aumente seus clientes utilizando uma ferramenta perfeita para atingir suas metas.
            </p>
          </div>
          <div class="tab-pane fade" id="themes">
            <p>
              Nós trabalhamos com várias temáticas, como por exemplo: <br><br> Lojas Virtuais, &nbsp;&nbsp; Blogs,
              &nbsp;&nbsp; Portfólios Online, &nbsp;&nbsp; Cardárpio Online, &nbsp;&nbsp; Sites de Notícias e até
              Sistemas de Gerenciamento de Estoque.
            </p>
          </div>
        </div>

      </div>

      <div class="col-xs-12 col-sm-6 col-md-4 col-lg-6">
        <img src="{{ asset('resources/images/others/question.png') }}" class="img-responsive questions-tw">
      </div>
    </div>

  </section>
  <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
       viewBox="0 0 842 73.4" style="enable-background:new 0 0 842 73.4;" xml:space="preserve">
<style type="text/css">
  .st0 {
    fill: #1A1A1A;
  }
</style>
    <g id="contact">
      <g>
        <path class="st0" d="M-10.5,0l863,0v61.9c0,0-15.4-11-102-10.9c-77.2,0.1-134.9,4.9-146.5,6c-1.4,0.1-2.7,0.3-4,0.5
			c-56.1,9.2-129.4,1.3-143.4-0.3c-1.5-0.2-2.9-0.4-4.3-0.7c-46-8.6-143.7-4.9-168.2-3.8c-3.4,0.1-6.8,0.5-10.2,1
			C130,75.9-10.5,53.1-10.5,53.1V0z"/>
      </g>
    </g>
</svg>
  <section class="contact">
    <div class="container" style="text-align: center">
      <h2 style="text-align: center;color: #c6c4c4;">Entre em contato</h2>
      Se você gostou do que viu até agora, você pode estar entrando em contato conosco!
      <br>Tem algum projeto para ser tornado realidade? Então entre em contato que nós fazemos o orçamento para você.

      <div style="padding-top: 20px">

        <form action="{{ url('contact/front') }}" method="post" role="form">
          {{ csrf_field() }}
          <div class="form-group center" style="text-align: -webkit-center;">
            <label>Nome Completo</label>
            <input type="text" class="form-control" placeholder=" Nome completo" style="max-width: 600px;" name="name" required>
          </div>

          <div class="form-group center" style="text-align: -webkit-center;">
            <label>Email</label>
            <input type="email" class="form-control" name="email" placeholder=" Endereço de email"
                   style="max-width: 600px;" required>
          </div>

          <div class="form-group center" style="text-align: -webkit-center;">
            <label>Digite sua mensagem</label>
            <textarea class="form-control" name="message" placeholder=" Digite uma mensagem"
                      style="max-width: 600px;height:120px;resize: none;padding-top: 14px;" required></textarea>
          </div>
          <button type="submit" class="btn btn-twuiseo"><i class="fa fa-paper-plane-o"></i> &nbsp; Enviar</button>
        </form>

      </div>
    </div>
  </section>
</main>


<footer class="footer-site">
  <svg style="margin-top: -10px"
       xmlns="http://www.w3.org/2000/svg"
       xmlns:xlink="http://www.w3.org/1999/xlink"
       width="110" height="110">
    <image x="0px" y="0px" width="110" height="110"
           xlink:href="data:img/png;base64,"/>
  </svg>
  <div class="row">
    <div class="container">

      <div class="testemonial-footer">
        A <b>TWUISEO </b>tenta trazer a todos um preço acessível para que você torne seu sonho em realidade. Nós
        trazemos o que há de melhor na questão de tecnologia e design. Fazemos todos os trabalhos com amor e carinho,
        para quando um usuário acessar seu site ele se sinta em casa.
      </div>

      <div class="social">
        <i class="fa fa-facebook-square fa-2x social-icon"></i>
      </div>
    </div>
  </div>
</footer>

<div class="clearfix"></div>

<div class="pull" id="footer"></div>

<script src="{{ asset('resources/js/jquery.js') }}"></script>
<script src="{{ asset('resources/js/bootstrap.min.js') }}"></script>
<script src="{{ asset('resources/js/bootsnav.js') }}"></script>
<script src="{{ asset('resources/js/scrolly.js') }}"></script>
<!-- Plugins -->
<script src="{{ asset('resources/plugins/leader/leader-line.min.js') }}"></script>
<script src="{{ asset('resources/plugins/jquery.typewriter.js') }}"></script>
<script src="{{ asset('resources/plugins/jquery-elevator/jquery.elevator.min.js') }}"></script>
<script src="{{ asset('resources/plugins/jquery-sweet-modal/jquery.sweet-modal.min.js') }}"></script>
<script src="{{ asset('js/script.min.js') }}"></script>
@if(Session::has('msg'))
  <script>

    alert('{{ Session::get('msg') }}');
  </script>
@endif

</body>
</html>